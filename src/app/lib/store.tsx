...
export function GameStateProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (youthCheckDays.includes(seasonDay)) {
      if (state.lastYouthArrivalSeason < seasonNumber || state.lastYouthArrivalDay < seasonDay) {
        ...
        updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), {
  ...
  const syncStats = useCallback((groupPlayers: any[]) => {
    ...
    if (state.lastProcessedSeason > 0 && globalSeason > state.lastProcessedSeason) {
      ...
      updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { leagueLevel: newLevel, wins: 0, draws: 0, losses: 0, points: 0, lastProcessedSeason: globalSeason, lastLeagueMatchDate: null, lastCupMatchDate: null, lastSeenMatchDay: 0, seasonResults: results, hasEliteTrophy: awardedTrophy || state.hasEliteTrophy });
      return; 
    }
  ...
  const addCredits = useCallback((amount: number) => {
    setState(s => {
      ...
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { inGameCurrency: newVal });
  ...
  const addCrystals = useCallback((amount: number) => {
    setState(s => {
      ...
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { crystals: newVal });
  ...
  const claimReward = useCallback((creditsReward: number, crystalsReward: number) => {
    ...
    setState(s => {
      ...
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { 
  ...
  const startArenaConstruction = useCallback((facility: any, cost: number) => {
    ...
    setState(s => {
      if (s.credits >= cost) {
        ...
        if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { inGameCurrency: newCredits, arena: sanitizeForFirestore(newArena) });
  ...
  const startHQConstruction = useCallback((facility: any, cost: number) => {
    ...
    setState(s => {
      if (s.credits >= cost) {
        ...
        if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { inGameCurrency: newCredits, hq: sanitizeForFirestore(newHQ) });
  ...
  const startBootcampConstruction = useCallback((facility: any, cost: number) => {
    ...
    setState(s => {
      if (s.credits >= cost) {
        ...
        if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { inGameCurrency: newCredits, bootcamp: sanitizeForFirestore(newBootcamp) });
  ...
  const startAcademyConstruction = useCallback((facility: any, cost: number) => {
    ...
    setState(s => {
      if (s.credits >= cost) {
        ...
        if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { inGameCurrency: newCredits, academy: sanitizeForFirestore(newAcademy) });
  ...
  const startMedicalConstruction = useCallback((facility: any, cost: number) => {
    ...
    setState(s => {
      if (s.credits >= cost) {
        ...
        if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { inGameCurrency: newCredits, medical: sanitizeForFirestore(newMedical) });
  ...
  const hireStaffMember = useCallback((member: StaffMember) => {
    setState(s => {
      ...
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { staff: sanitizeForFirestore(updatedStaff), inGameCurrency: newCredits });
  ...
  const trainStaffSkill = useCallback((role: StaffRole, skillKey: 'primary' | 'secondary', cost: number) => {
    ...
    setState(s => {
      ...
      success = true;
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { staff: sanitizeForFirestore(updatedStaff), crystals: newCrystals });
  ...
  const assignToRole = useCallback((slot: LineupSlot, heroId: string | null) => {
    setState(s => {
      ...
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { lineup: newLineup });
  ...
  const updateTactics = useCallback((strategy: string, lineSettings: { carry: string; mid: string; offlane: string }) => {
    setState(s => {
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { strategy, lineSettings });
  ...
  const markMatchAsSeen = useCallback((day: number) => {
    setState(s => {
      ...
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { lastSeenMatchDay: day });
  ...
  const dismissSeasonResults = useCallback(() => {
    setState(s => {
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { seasonResults: null });
  ...
  const updateProfileName = useCallback((newName: string) => {
    setState(s => {
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { displayName: newName });
      return s;
    });
  }, [user, db]);
  ...
  const updateProfileCountry = useCallback((newCountry: string) => {
    setState(s => {
      if (user) updateDocumentNonBlocking(doc(db, 'players_v5', user.uid), { country: newCountry });
