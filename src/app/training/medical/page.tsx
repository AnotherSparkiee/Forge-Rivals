...
export default function MedicalPage() {
  const { 
    medical, credits, crystals, startMedicalConstruction, checkConstructions, language, isLoaded 
  } = useGameState();
  ...
  const handleFacilityUpgrade = () => {
    ...
    if (startMedicalConstruction(selectedFacility as any, cost, crew.multiplier, crew.price)) {
        ...
        {selectedFacility && (
          <DialogContent className="max-w-md bg-card border-white/10 p-0 overflow-hidden shadow-2xl border">
              ...
              <div className="grid grid-cols-2 gap-3 mt-6">
                ...
                <div className="bg-secondary/30 p-3 rounded-xl text-center border border-white/5">
                   ...
                   <p className="text-sm font-bold text-primary flex items-center justify-center gap-1">
                     ...
                     {Math.ceil((4 * (((medical as any)[selectedFacility] || 0) + 1)) / (CREWS.find(c => c.id === selectedCrewId)?.multiplier || 1))} {t.hours}
                   </p>
                </div>
              </div>
            </div>
            ...
            <DialogFooter className="p-4 bg-secondary/20 border-t border-white/5">
              <Button className="w-full h-14 hero-gradient font-black text-[10px] uppercase tracking-widest shadow-xl shadow-primary/20 active:scale-95 transition-all" onClick={handleFacilityUpgrade} disabled={isAnyConstructing}>
