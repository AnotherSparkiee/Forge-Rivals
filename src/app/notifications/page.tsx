...
export default function NotificationsPage() {
  ...
  const handleMarkAsRead = (id: string) => {
    updateDocumentNonBlocking(doc(db, 'notifications_v1', id), { read: true });
  };
  ...
  const handleMarkAllRead = async () => {
    ...
    unread.forEach(n => {
      batch.update(doc(db, 'notifications_v1', n.id), { read: true });
    });
    await batch.commit();
  };
  ...
  const handleClearAll = async () => {
    ...
    notifications.forEach(n => {
      batch.delete(doc(db, 'notifications_v1', n.id));
    });
    await batch.commit();
  };
