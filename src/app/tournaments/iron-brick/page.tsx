...
const REG_CLOSE_TIME = "21:20"; 
const MAX_PARTICIPANTS = 16;
const TOUR_ID = 'iron-brick';

export default function IronBrickPage() {
  ...
  const participantsQuery = useMemoFirebase(() => {
    return query(collection(db, 'players_v11'), where('tournaments', 'array-contains', TOUR_ID));
  }, [db]);

  const { data: participants, isLoading: isParticipantsLoading } = useCollection(participantsQuery);

  const isJoined = useMemo(() => {
    if (!profile?.tournaments?.includes(TOUR_ID)) return false;
    return true;
  }, [profile]);
  ...
  useEffect(() => {
    if (hasFinished && isJoined && !finalResultRef.current && userRef && profile && tournamentData) {
      ...
      const mockResult = {
        scoreA: 1, scoreB: 1,
        matchSummary: "Solid performance in the Brick arena.",
        teamStats: { teamA: { kills: 18, towersDestroyed: 7 }, teamB: { kills: 18, towersDestroyed: 7 } },
        heroPerformance: []
      };
      
      recordMatch(mockResult.scoreA > mockResult.scoreB ? (profile.displayName || "Manager") : (mockResult.scoreA === mockResult.scoreB ? "Draw" : (opponent?.name || "Rival")), mockResult, 0, opponent?.name || "Tournament Rival", 'tournament', mskNow.toISOString());
