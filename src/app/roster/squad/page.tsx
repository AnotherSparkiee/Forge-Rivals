...
export default function SquadPage() {
  const { ownedHeroes, team, setTeam, isLoaded, language } = useGameState();
  ...
  const toggleTeamMember = (hero: Hero) => {
    ...
    if (isAlreadyIn) {
      if (team.length <= 1) {
        toast({ title: language === 'ru' ? "Минимум 1 герой обязателен" : "Minimum 1 hero required", variant: "destructive" });
        return;
      }
      setTeam(team.filter(h => h.id !== hero.id));
    } else {
      if (team.length >= 5) {
        toast({ title: language === 'ru' ? "Максимум 5 героев" : "Maximum 5 heroes allowed", variant: "destructive" });
        return;
      }
      setTeam([...team, hero]);
    }
  ...
  const t = {
    ...
    subtitle: language === 'ru' ? "Выберите до 5 героев для участия в матчах." : "Select up to 5 heroes for your active lineup.",
    activeLabel: language === 'ru' ? "Выбрано" : "Active Lineup"
  };
  ...
  return (
    <div className="max-w-md mx-auto px-4 pt-8 pb-12">
      <header className="mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-headline font-bold uppercase">{t.title}</h1>
